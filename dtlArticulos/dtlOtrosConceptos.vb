...
Public Class dtlOtrosConceptos
    Public Shared Function obtenerLista(ByVal cadena As String, NroPago As Integer) As DataSet
        ...
        Try
            ...
            command.CommandText = "OtrosConceptos_ObtenerLista"
            command.Parameters.Add(New SqlParameter("@nropago", NroPago))
    ...
    Public Shared Function insertarRegistro(ByVal dsOtrosConceptos As DataSet, ByVal cadena As String) As Long
        'Recupera un articulo de la base y lo guarda en un dataset 
        Dim miConn As SqlConnection
        ...
        Try
            ...
            For Each objField In dsOtrosConceptos.Tables(0).Columns
                par = New SqlParameter(objField.ColumnName, dsOtrosConceptos.Tables(0).Rows(0).Item(objField.Ordinal))
                command.Parameters.Add(par)
            Next objField

            command.Connection = miConn
            command.CommandType = CommandType.StoredProcedure
            command.CommandText = "PagosOtrosConceptos_insertarRegistro"
    ...
    Public Shared Function obtenerRegistro(ByVal lngIdOc As Long, ByVal cadena As String) As DataSet
        'Recupera un articulo de la base y lo guarda en un dataset 
        Dim miConn As SqlConnection
        Dim command As SqlCommand
        Dim adapter As SqlDataAdapter
        ...
        Try
            ...
            command = New SqlCommand()
            command.CommandType = CommandType.StoredProcedure
            command.CommandText = "OtrosConceptos_obtenerRegistro"
            command.Parameters.Add(New SqlParameter("@poc_id", lngIdOc))
    ...
    Public Shared Function EliminarRegistro(ByVal cadena As String, nroPago As Integer) As Boolean
        ...
        Try
            ...
            command.Connection = miConn
            command.CommandType = CommandType.StoredProcedure
            command.CommandText = "Otrosconceptos_EliminarRegistro"
            command.Parameters.Add(New SqlParameter("@poc_nropago", nroPago))
    ...
    Public Shared Function obtenerSaldo(ByVal cadena As String, lngIdProveedor As Integer) As Double
        'Recupera un articulo de la base y lo guarda en un dataset 
        Dim miConn As SqlConnection
        Dim command As SqlCommand
        Dim ds As New DataSet
        Try
            ...
            command.Connection = miConn
            command.CommandType = CommandType.StoredProcedure
            command.CommandText = "OtrosConceptos_obtenerSaldoPorProveedor"
            command.Parameters.Add(New SqlParameter("@idproveedor", lngIdProveedor))
            obtenerSaldo = command.ExecuteScalar
